...
class NutritionAgent extends baseAgent_1.BaseAgent {
    constructor() {
        super('Nutrition Agent', 'Diabetes Nutrition Specialist', `You are an AI nutritionist specializing in diabetes dietary management. 
      Your role is to:
    ...
    parseResponse(response) {
        ...
        const nutritionDisclaimer = "Individual responses to foods may vary. Monitor your blood sugar to understand how different foods affect you personally. " +
    ...
    extractFoodMentions(response) {
        ...
        [...lowGIFoods, ...highGIFoods].forEach(food => {
            if (response.toLowerCase().includes(food)) {
                ...
                mentions.push(`- ${food}: ${isLowGI ? 'Low' : 'High'} glycemic impact - ${isLowGI
